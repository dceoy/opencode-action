...
This is a strictly non-mutating repository review. Never edit, create, delete, format, or generate repository files. Do not run repository QA commands or skills that may mutate files. Do not use `--fix`, `--write`, safe-fix modes, or equivalent write-enabled modes. Never run `git add`, `commit`, `push`, `reset`, `restore`, `checkout`, `switch`, `merge`, `rebase`, or similar mutating commands. Treat these as behavioral safeguards: the workflow token's `contents: read` permission is the security boundary.
...
- **Credential precedence, exact:** when `use-github-token` is `true`, the caller's original `GH_TOKEN`/`GITHUB_TOKEN` is the exclusive credential for reads and writes; no App-token candidate is inspected or exported. When it is not `true`, every resolved candidate is tried and the first one that verifies as `opencode-agent[bot]` is exported; if none verifies, the run fails. Structured PR review **submissions** (step 7) call `opencode_require_app_token_for_review "${USE_GITHUB_TOKEN:-false}" "${GITHUB_REPOSITORY}" "${PR_NUMBER}"` immediately before every write to enforce this policy.
