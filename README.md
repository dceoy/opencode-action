...
**No automatic branch commit/push in `review-only`.** In `review-only` mode the action runs plain `opencode run` instead of `opencode github run`. `opencode github run` is OpenCode's GitHub-integration entrypoint that can commit and push a branch on its own initiative after the agent returns; plain `opencode run` has no branch-management behavior at all — it only runs the agent over the `/review-pr` prompt and returns. Mutation-capable workflows (`review-only: false`, for example `/oc ...`) keep using `opencode github run` unchanged. As additional defense in depth, before OpenCode starts the action fails the whole job if the checkout is not pristine (any unstaged, staged, untracked, deleted, renamed, or intent-to-add change, or a dirty submodule), and the checkout workflow uses `persist-credentials: false` so the checkout carries no git-credential a `git push` could use.

**Trusted write targeting.** In `review-only` mode the model never chooses what the review writes to. `review-pr-worktree-guard.sh init` records, in a per-run state directory outside the checkout (under `RUNNER_TEMP`, mode `0700`), a snapshot of the pristine worktree bound to the repository, PR number, head commit SHA, and run identifiers read from the trusted GitHub Actions event context (`GITHUB_REPOSITORY`, `GITHUB_EVENT_PATH`, `GITHUB_RUN_ID`, `GITHUB_RUN_ATTEMPT`). `review-pr-submit.sh` derives the repository, PR number, and target commit from that same trusted context — not from any argument the agent supplies — and, immediately before it acquires any write-capable credential, independently re-verifies the worktree against the recorded snapshot and re-validates the binding (refusing a missing/malformed/stale state, a moved head SHA, or state from another repo, PR, run, or attempt). An `update` targets only the review ID the same run recorded when `submit-initial` succeeded. The write-capable token is resolved and used only inside `review-pr-submit.sh` at the moment of the `gh api` call.
